# test_that("dbernstein has the correct derivatives. ", {
...
#                (lambda[m-1]-2*lambda[m] + lambda[m+1])*(m+1)*m, tolerance = 1e-8)
#   expect_equal(c(numDeriv::hessian(function(x) dbernstein(x, lambda),x = 0)),
#                (lambda[3] - 2*lambda[2] + lambda[1])*(m+1)*m, tolerance = 1e-3)
# })
#
